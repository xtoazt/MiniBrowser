...
export default function App() {
  ...
  const toggleTheme = () => {
    ...
    document.body.className = theme === "dark" ? "bg-white text-black" : "bg-gray-900 text-white";
  };
  ...
  const bookmark = () => {
    if (url && !bookmarks.includes(url)) setBookmarks((b) => [...b, url]);
  };
    ...
    <div className={\`min-h-screen p-4 transition \${theme === "dark" ? "bg-gray-900 text-white" : "bg-white text-black"}\`}>
      <div className="max-w-6xl mx-auto space-y-4">
        <div className="flex items-center space-x-2">
          ...
          <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://example.com" className="flex-grow px-4 py-2 rounded bg-gray-100 text-black" />
        ...
        <div className="border rounded-lg overflow-hidden bg-white">
          {loading ? (
            ...
            <iframe sandbox="allow-scripts allow-same-origin" srcDoc={iframeSrc} className="w-full h-[600px] border-none" title="Rendered Site" />
          ) : (
